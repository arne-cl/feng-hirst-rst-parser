...
def cal_feat_imp(label,sample):
	...
	score_tuples = score_dict.items()
	score_tuples.sort(value_cmpf)
...
def cal_Fscore(labels,samples):
	...
	for p in range(len(samples)): # for every data point
		...
		if p_num.has_key(label): p_num[label] += 1
		else: p_num[label] = 1
